...
from src.core.models import CreatePlant, Plant, User
...
@router.get("/plants", tags=["Plant"], response_model=List[Plant])
...
@router.get("/plants/{plant_id}", tags=["Plant"])
async def get_plant(
    current_user: Annotated[User, Depends(get_current_active_user)],
    plant_id: str,
    db: Redis = Depends(redis_connection),
...
@router.put("/plants/{plant_id}", tags=["Plant"])
